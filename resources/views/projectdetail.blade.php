...
<link rel="stylesheet" href="{{asset('/vendors/jquery-bar-rating/bars-1to10.css')}}">
...
<div class="row">
    <div class="col-md-9 grid-margin">
        <div class="card">
            <div class="card-body">
                ...
                <h5 class="mt-2 font-weight-bold">Tệp đính kèm: </h5>
                <a href="{{asset($project->file_uploaded->path)}}">{{$project->file_uploaded->name}}</a>
                @endif
            ...
            <div class="card-body">
                ...
                <div class="row py-4 border-bottom">
                    ...
                    <div class="col-4 text-right">
                        <h4 class="font-weight-bold mb-4">${{$bidding->bid_amount}} VND</h4>
    ...
    <div class="col-md-3 grid-margin">
                ...
                <div class="table-responsive">
                    <table class="table table-borderless">
                        <tbody>
                            @if(Auth::user()->id != $project->user_id)
                            <tr>
                                <td class="text-muted pl-0 pr-1 font-weight-bold">Chủ dự án:</td>
                                <td class="text-muted px-0"><a href="{{route('profile',$project->user_id)}}">{{$project->username}}</a></td>
                            </tr>
                            @endif
                            <tr>
                                <td class="text-muted pl-0 pr-1 font-weight-bold">Người đảm nhận:</td>
                                <td class="text-muted px-0"><a href="{{route('profile',$biddings->user_id)}}">{{$biddings->username}}</a></td>
                            </tr>
                            <tr>
                    ...
                    <table class="table table-borderless">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-muted px-0 ">{{$project->average_bid}} VND</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
        ...
        @if($project->state === 'completed' || $project->ended_at === 0&&(Auth::user()->id===$project->freelancer_id||Auth::user()->id===$project->user_id))
        <div class="card mt-3">
            <div class="card-body">
                @if(Auth::user()->id == $project->freelancer_id)
                <div class="row pb-2 border-bottom">
                    <h4 class="font-weight-bold col-12">Đánh giá chủ dự án</h4>
                </div>
                @endif
                @if(Auth::user()->id == $project->user_id)
                <div class="row pb-2 border-bottom">
                    <h4 class="font-weight-bold col-12">Đánh giá người đảm nhận</h4>
                </div>
                @endif
                <form class="forms-sample" name="rating" method="POST" action="{{route('rating',$project->id)}}" id="ratingForm">
                    <input type="hidden" name="_token" value="{{@csrf_token()}}">
                    @if(Auth::user()->id == $project->freelancer_id)
                    <input type="hidden" name="rating_user" value="{{$project->freelancer_id}}">
                    <input type="hidden" name="rated_user" value="{{$project->user_id}}">
                    @endif
                    @if(Auth::user()->id == $project->user_id)
                    <input type="hidden" name="rating_user" value="{{$project->user_id}}">
                    <input type="hidden" name="rated_user" value="{{$project->freelancer_id}}">
                    @endif
                    <div class="form-group my-2">
                        <div class="br-wrapper br-theme-bars-1to10">
                            <select id="example-1to10" name="rating" autocomplete="off" class="d-none">
                                @for($i = 1; $i <= 10; $i++) @if(!is_null($rating) && $i==$rating->points)
                    ...
                    @if(is_null($rating))
                    <button class="btn btn-primary btn-sm">Lưu đánh giá</button>
                    @endif
                </form>
            </div>
        </div>
...
<script>
    $(function() {
        $('#example-1to10').barrating('readonly', true);
    });
</script>
@endif
